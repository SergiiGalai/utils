...
class GoogleDriveFileConverterTests(unittest.TestCase):
   ...
   def test_CloudFileMetadata_when_converted_gfile_with_subpath(self):
      ...
      #act
      actual = self.sut.convert_GoogleDriveFile_to_CloudFileMetadata(gFile, '/sub')
      #assert
      self.assertEqual(actual, expected)
   ...
   def test_CloudFileMetadata_when_converted_gfile_with_level2_subpath(self):
      ...
      #act
      actual = self.sut.convert_GoogleDriveFile_to_CloudFileMetadata(gFile, '/sub/sub2')
      #assert
      self.assertEqual(actual, expected)
   ...
   def _createGoogleDriveFile(mimeType='application/pdf', name=DEFAULT_NAME, id=DEFAULT_ID):
      return GoogleDriveFile(metadata={'id':id, 'title':name, 'modifiedDate':'2023-08-15T14:27:44.000Z', 'mimeType':mimeType, 'fileSize': '12345'})

   @staticmethod
   def _createCloudFileMetadata(cloud_path=DEFAULT_NAME, name=DEFAULT_NAME, id=DEFAULT_ID):
      return CloudFileMetadata(name, cloud_path, datetime.datetime(2023, 8, 15, 14, 27, 44), 12345, id, '0')

   @staticmethod
   def _createGoogleDriveFolder(name='Settings', id=DEFAULT_ID):
      return GoogleDriveFile(metadata={'id':id, 'title':name, 'mimeType':'application/vnd.google-apps.folder'})
