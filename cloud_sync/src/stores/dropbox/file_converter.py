...
class DropboxFileConverter:
    ...
    def convert_dropbox_entries_to_FileMetadatas(self, dropbox_entries: list) -> ListCloudFolderResult:
        ...
        for entry in dropbox_entries:
            self._logger.debug("entry path_display=`{}`, name={}".format(
                entry.path_display, entry.name))
            if self.__isFile(entry):
                cloud_file = self.convert_DropboxFileMetadata_to_CloudFileMetadata(
                    entry)
                result.files.append(cloud_file)
            else:
                cloud_folder = self.convert_DropboxFolderMetadata_to_CloudFolderMetadata(
                    entry)
