...
class FileSyncronizationService:
    ...
    def __map_cloud_files_to_local(self, local_root: str, local_files: list, cloud_root: str, cloud_filemetadatas: list):
        ...
        for cloud_file_md in cloud_filemetadatas:
            ...
            local_path = self.localFileStore.get_absolute_path(cloud_path)
            ...
            if name in local_files:
                ...
                if self.__are_equal_by_metadata(local_md, cloud_file_md):
                    self.logger.info('file {} already synced [by metadata]. Skip'.format(cloud_path))
                else:
                    self.logger.info('file {} exists with different stats. Downloading temporary'.format(cloud_path))
                    if self.__are_equal_by_content(local_path, cloud_path):
                        self.logger.info('file {} already synced [by content]. Skip'.format(cloud_path))
                    else:
                        if local_md.client_modified > cloud_file_md.client_modified:
                            self.logger.info('file {} has changed since last sync (cloud={} < local={}) => upload list'
                                .format(local_path, cloud_file_md.client_modified, local_md.client_modified))
                            upload_list.append(cloud_path)
                        else:
                            self.logger.info('file {} has changed since last sync (cloud={} > local={}) => download list'
                                .format(cloud_path, cloud_file_md.client_modified, local_md.client_modified))
                            download_list.append(cloud_path)
            else:
                self.logger.info('file NOT found locally - {} => download list'.format(cloud_path))
                download_list.append(cloud_path)
    ...
    def __are_equal_by_metadata(self, local_md: LocalFileMetadata, cloud_md: CloudFileMetadata):
        ...
        if not equal_by_name:
            self.logger.info('file diff by name: local={}, remote={}'.format(local_md.name, cloud_md.name))
            return False
        if not equal_by_size:
            self.logger.info('file={} diff by size: local={}, remote={}'.format(local_md.name, local_md.size, cloud_md.size))
            return False
        if not equal_by_date:
            self.logger.info('file={} diff by date: local={}, remote={}'.format(local_md.name, local_md.client_modified, cloud_md.client_modified))
            return False
        return True
