...
class GoogleDriveFileConverterTests(unittest.TestCase):
    ...
    def test_2_files_when_converterd_gfiles_with_2_files_in_the_subfolder(self):
        gFile1 = self.__create_drive_file(id='1', name='f1.pdf')
        gFile2 = self.__create_drive_file(id='2', name='F2.pdf')
        expected1 = self.__create_cloud_file('/folder/folder2/f1.pdf', 'f1.pdf', '1')
        expected2 = self.__create_cloud_file('/folder/folder2/F2.pdf', 'F2.pdf', '2')
        # act
    ...
    def test_CloudFileMetadata_when_converted_gfile_with_file_size(self):
        ...
        expected = self.__create_cloud_file()
        # act
    ...
    def test_CloudFileMetadata_when_converted_gfile_with_subpath(self):
        gFile = self.__create_drive_file()
        expected = self.__create_cloud_file('/sub/File1.pdf')
        # act
    ...
    def test_CloudFileMetadata_when_converted_gfile_with_level2_subpath(self):
        gFile = self.__create_drive_file()
        expected = self.__create_cloud_file('/sub/sub2/File1.pdf')
        # act
    ...
    @staticmethod
    def __create_drive_file(mimeType='application/pdf', name=DEFAULT_NAME, id=DEFAULT_ID):
    ...
    @staticmethod
    def __create_cloud_file(cloud_path=DEFAULT_NAME, name=DEFAULT_NAME, id=DEFAULT_ID):
        return CloudFileMetadata(name, cloud_path,
                                 datetime.datetime(2023, 8, 15, 14, 27, 44),
                                 12345, id, '0')

    @staticmethod
    def __create_drive_folder(name='Settings', id=DEFAULT_ID):
        return GoogleDriveFile(metadata={
            'id': id, 'title': name,
            'mimeType': 'application/vnd.google-apps.folder'})

    @staticmethod
    def __create_cloud_folder(cloud_path='Settings', name='Settings', id=DEFAULT_ID):
